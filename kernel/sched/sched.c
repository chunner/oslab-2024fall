...
void setup_process_stack(pcb_t *pcb, int argc, char *argv[]) {
    ...
    *(int64_t *) user_sp_kva = (int64_t) argc;
    ...
    check_uva_mem(argv, sizeof(char *) * argc, current_running);
    ...
    for (int i = 0; i < argc; i++) {
        check_uva_mem(argv[i], 64, current_running);
...
pid_t do_exec(char *name, int argc, char *argv[]) {
    ...
    check_uva_mem(name, 32, current_running);
    int taskid;
    ...
    create_PageNode(pcb[pcb_id].pgdir, pcb[pcb_id].pgdir, (uintptr_t) PGDIR_VA, &pcb[pcb_id]);
